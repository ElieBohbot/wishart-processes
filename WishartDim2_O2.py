...
def WIS2_O2_StepByStep_e1(a,X,t_1,t_2):
    ...
    if X[1][1]==0:
        ...
        res[1][0]=X[1][0]
        res[0][1]=X[0][1]
        res[0][0]= CIR_O2_StepByStep(a,t_2-t_1, X[0][0])
    else:
        U = np.zeros((2,2))
...
def WIS2_O2_array_e1(a,x,T,N):
    t = np.linspace(0,T,N+1)
    res = np.zeros((N+1,2,2))
    res[0] = x
    for i in np.arange(N):
        res[i+1] = WIS2_O2_StepByStep_e1(a,res[i],t[i],t[i+1])
    return res

#==============================================================================
# Algorithm 2 : Order 2 scheme for the simulation of WIS_2(x,a,0,I_2;t)
#==============================================================================  
    
def WIS2_O2_StepByStep_I2(a,X,t_1,t_2):
    y = X
    y = WIS2_O2_StepByStep_e1(a,y,t_1,t_2)
    p = np.array([[0,1],[1,0]])
    Y = WIS2_O2_StepByStep_e1(a,py.dot(py.dot(p,y),p),t_1,t_2)
    y = py.dot(py.dot(p,Y),p)
    return y
    
def WIS2_O2_array_I2(a,x,T,N):
    t = np.linspace(0,T,N+1)
    res = np.zeros((N+1,2,2))
    res[0] = x
    for i in np.arange(N):
        res[i+1] = WIS2_O2_StepByStep_I2(a,res[i],t[i],t[i+1])
    return res
    
    
#==============================================================================
# Algorithm 3 : Order 2 scheme for the simulation of WIS_2(x,a,b,A;t)
#============================================================================== 

    
#on suppose que A=u*I_2 et b=v*I_2  
def WIS2_O2_StepByStep(a,X,t_1,t_2,u,v):
    #Calcul de q_t
    t = t_2 - t_1
    ...
    Y = WIS2_O2_StepByStep_I2(a,x_prime,t_1,t_2)
    return np.dot(c,np.dot(Y,c))
    

def WIS2_O2_array(a,x,T,N,u,v):
    t = np.linspace(0,T,N+1)
    res = np.zeros((N+1,2,2))
    res[0] = x
    for i in np.arange(N):
        res[i+1] = WIS2_O2_StepByStep(a,res[i],t[i],t[i+1],u,v)
    return res       
